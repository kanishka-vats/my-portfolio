...
<div class="section-padding centered-view">
  ...
  <div class="bio-box">
    {#each sections as sec}
      <p on:click={() => toggleExpand(sec.id)} style="cursor: pointer; margin-bottom: 15px">
        <span class="highlight">{sec.title}</span>:{' '}
        {#if expanded[sec.id]}
          <span style="white-space: pre-wrap">{sec.fullText}</span>
        {:else}
          <span>
            {sec.preview} <span class="dim" style="font-size: 0.85em">... (to view more click here)</span>
          </span>
        {/if}
      </p>
    {/each}
  </div>
  <button class="back-btn mt-20" on:click={onBack}>[ ESC ] RETURN</button>
</div>
