...
const BlogSection = ({ onBack }) => {
  ...
  const handleRead = (e) => {
    if (e.key === 'Enter') {
      if (blogPosts[addressInput]) setSelectedPost(blogPosts[addressInput]);
      ...
      {!selectedPost ? (
        <>
          <div className="centered-grid">
            {Object.keys(blogPosts).map(addr => (
              <div key={addr} className="memory-block">
          ...
          <div className="input-line centered-input mt-20">
            ...
            <input className="term-input" autoFocus value={addressInput}
              onChange={(e) => setAddressInput(e.target.value)} onKeyDown={handleRead} />
          </div>
        </>
      ) : (
        <div className="bio-box">
          <h3 className="highlight">{selectedPost.title}</h3>
          <p>{selectedPost.content}</p>
          <button className="back-btn mt-20" onClick={() => setSelectedPost(null)}>[ BACK TO LIST ]</button>
        </div>
      )}
      <button className="back-btn mt-20" onClick={onBack}>[ ESC ] EXIT BLOG</button>
    </div>
  );
};
